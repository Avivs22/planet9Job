...
export function ViewAnalystDialog(props: ViewAnalystDialogProps) {
  const [formValues, setFormValues] = useState<
    FormValues<FormValue<string>> | undefined
  >(undefined);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("Form submitted successfully!", {
        progressStyle: {
          background: "linear-gradient(to left, #6ab4ff, #c2a6ff)",
        },
      });
    } catch (error) {
      console.log(error);
        ...
        {formValues && (
          <>
            ...
            <ToggleButtonGroup
              ...
              exclusive
              onChange={(event, newClassification) =>
                handleClassificationChange("classification", newClassification)
              }
            ...
            <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
              {(formValues.filters as FormValue<string>).item?.map(
                (filter: FormValueItem<string>) => (
                  <Chip
                    key={filter.value}
                    label={filter.value}
                    clickable
