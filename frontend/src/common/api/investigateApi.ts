...
export function useIPInfoQuery(params: GetBaseQueryParams, enabled: boolean) {
  return useQuery(
    ...
    async () => {
      ...
      // })
      // try {
      return (await axios.get("/api/athena/ip-info", { params }))
        .data as GetIPInfoQueryResponse[];
      // } catch (error: any) {
...
export function useFaviconQuery(params: GetBaseQueryParams) {
  return useQuery(
    ...
    async () => {
      // try {
      return (await axios.get("/api/athena/favicon", { params }))
        .data as GetFaviconQueryResponse;
      // } catch (error: any) {
      //   if (axios.isAxiosError(error) && error.response) {
      //     const message = error.response.data?.detail || error.response.data;
      //     return message;
...
export function usePredictV2Query(
  ...
  return useQuery(
    ["predict-v2", params],
    async () => {
      return (await axios.post("/api/v2/predict", params))
        .data as PredictV2Response;
    },
    {
      enabled,
      refetchOnWindowFocus: false,
...
export function useScreenshotQuery(params: GetBaseQueryParams) {
  return useQuery(
    ...
    async () => {
      // try {
      return (await axios.get("/api/athena/screenshot", { params }))
        .data as GetScreenshotQueryResponse;
      // } catch (error: any) {
      //   if (axios.isAxiosError(error) && error.response) {
      //     const message = error.response.data?.detail || error.response.data;
      //     return message;
...
export function useLinksQuery(params: GetLinksQueryParams, enabled: boolean) {
  return useQuery(
    ...
    async () => {
      switch (params.type) {
        case "Outer Links":
          return (await axios.get("/api/athena/outer-links", { params })).data
            .info as GetScreenshotQueryResponse;
        case "Additional Analysis":
          return new Promise<{ [key: string]: string } | string[]>(
            (res, rej) => {
              // try {
  ...
  return useQuery(
    ...
    async () => {
      return new Promise<object>((res, rej) => {
        setTimeout(() => {
          res(
            Object.entries(stubConfig.DNS).reduce((acc: any, [key, value]) => {
...
export function useModelInferenceQuery(params: GetBaseQueryParams) {
  return useQuery(
    ["model_inf", params],
    async () => {
      ...
      //   }, 3000)
      // })
      return (await axios.get("/api/athena/scan", { params }))
        .data as GetIPInfoQueryResponse[];
    },
    {
      enabled: true,
