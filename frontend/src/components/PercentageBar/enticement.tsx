...
import { useGetBatchEnticementInfoQuery, useGetBatchInferenceInfoQuery, useGetBatchOodInfoQuery, useGetBatchStatusInfoQuery } from '../../common/api';
  ...
  return (            
    <Grid item xs={12} sm={6} >
        ...
        <div className="progress-bar">
            <div className="progress-bar-part adult-content" style={{ width: `${adultContentDatingPercentage}%`}}>{adultContentDatingPercentage}%</div>
            <div className="progress-bar-part finance-banking" style={{ width: `${financeBankingPercentage}%`}}>{financeBankingPercentage}%</div>
            <div className="progress-bar-part job-scam" style={{ width: `${jobScamPercentage}%`}}>{jobScamPercentage}%</div>
            <div className="progress-bar-part business-ecommerce" style={{ width: `${businessEcommercePercentage}%`}}>{businessEcommercePercentage}%</div>
            <div className="progress-bar-part other" style={{ width: `${otherPercentage}%`}}>{otherPercentage}%</div>
            <div className="progress-bar-part benign" style={{ width: `${benignEcommercePercentage}%`}}>{benignEcommercePercentage}%</div>
            <div className="progress-bar-part url-ood" style={{ width: `${urlOodEcommercePercentage}%`}}>{urlOodEcommercePercentage}%</div>
            <div className="progress-bar-part not-inferenced-yet" style={{ width: `${notInferencedEcommercePercentage}%`}}>{notInferencedEcommercePercentage}%</div>            
        </div>
