...
import {AccountCircle, ArrowBack, ArrowForward, ArrowLeft, Close} from "@mui/icons-material";
...
function useAddSDBCommentMutation() {
  ...
  return useMutation<AddSDBCommentResponse, unknown, AddSDBCommentParams>({
    mutationFn: async (params: AddSDBCommentParams) => {
      return (await axios.post('/api/sdb/add-comment', params)).data as AddSDBCommentResponse;
    },
    onSuccess() {
  ...
  return (
    <Card sx={{p: 1.5}}>
      <Stack direction="row" spacing={2}>
        <Avatar noName orientation="vertical" size={48} analyst={props.analyst} />
        ...
        <Stack spacing={1}>
          ...
          <Typography fontFamily="Helvetica Medium" variant="body2" sx={{color: '#ffffff90'}}>
...
export function ViewSocialMediaItemDialog(props: ViewSocialMediaItemDialogProps) {
  const [uuid, setUuid] = useState<string | undefined>(props.uuid);
  const [paging, setPaging] = useState<ItemPagingInfo | undefined>(props.paging);
  useEffect(() => {
    setUuid(props.uuid);
  ...
  const uploadDate = useMemo(() => data?.item?.added_at ? new Date(data.item.added_at * 1000) : 0, [data?.item?.added_at]);
      ...
      <Box
        ...
        {data?.item && (
          <Grid container spacing={6} sx={{position: 'relative',}}>
            <Grid item xs={7}>
              <Stack sx={{ height: '100%', overflow: 'auto', userSelect: 'none' }} justifyContent="center" alignItems="center">
                <ItemSnapshot item={data.item} scraper={data.scraper ?? undefined}/>
              </Stack>
            </Grid>
            ...
            <Grid item xs={5}>
              <Box
                sx={{
                  width: '0px !important',
                  height: props.paging ? 'calc(100% - 80px)' : 'calc(100% - 48px)',
                  position: 'absolute',
              ...
              <Stack direction="row" sx={{mt: 3}} spacing={2} alignItems="center">
                ...
                <Avatar orientation="vertical" size={42} analyst={data.item.analyst}/>
                ...
                <Box>
                  {uploadDate && (
                    <Typography fontFamily="Helvetica Medium">
                      ...
                      <Typography component="span" sx={{ml: 1}}>
                        {format(uploadDate, 'MMMM dd yyyy')} <b>{format(uploadDate, 'HH:mm')}</b>
                      </Typography>
                    </Typography>
                  )}
                  ...
                  <Typography fontFamily="Helvetica Medium">
                    Username:
                    <Typography component="span" sx={{ml: 1, color: '#01a1fd'}}>
                      {data.item.user}
                    </Typography>
                  </Typography>
                </Box>
              ...
              <Stack direction="row" alignItems="center" spacing={2} sx={{mt: 7}}>
                <Typography
                  fontFamily="Helvetica Medium"
                  variant="h6"
                >
              ...
              >
                {data.comments.map(c => (
                  <CommentBox key={c.uuid} text={c.text} timestamp={c.added_at} analyst={c.analyst} />
                ))}
