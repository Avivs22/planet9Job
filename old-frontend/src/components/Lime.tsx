...
export function LimeTokenBars(props: LimeTokenBarsProps) {
  ...
  const computed = useMemo(() => {
    ...
    let lineLength = (barWidth + barMargin) * data.tokenScores.length - barMargin;
    if (orientation == 'vertical') {
      // make space for first label
      lineLength += barMargin;
    }

    const lineStart = orientation == 'horizontal' ? (rect.width / 2 - lineLength / 2) : (rect.height / 2 - lineLength / 2);
  ...
  useEffect(() => {
    ...
    if (orientation == 'horizontal')
      svg.attr('viewBox', `${computed.lineStart - 15} 0 ${computed.lineLength + 30} ${rect.height}`);
    else
      svg.attr('viewBox', `0 ${computed.lineStart - 15} ${rect.width} ${computed.lineLength + 30}`);

    // compute overflow factor
    if (props.onOverflow) {
      const factor = orientation == 'horizontal'
    ...
    const mainG = svg.append('g')
      .classed('token-importance-group', true);

    mainG.append('g')
      .classed('text-group-1', true);
    mainG.append('g')
      .classed('text-group-2', true);
    mainG.append('g')
  ...
  useEffect(() => {
    ...
    const bars = barG.selectAll('rect')
      ...
      .join('rect')
      .transition()
      .attr('fill', d => d.importance > 0 ? CLASS_COLORS[1] : CLASS_COLORS[0]);

    if (orientation == 'horizontal') {
      bars.attr('x', (_, i) => computed.lineStart + i * (barWidth + barMargin))
        .attr('y', d =>
          d.importance > 0
      ...
      bars.attr('x', d =>
        ...
        .attr('y', (_, i) => computed.lineStart + i * (barWidth + barMargin) + barMargin)
        .attr('width', d => Math.abs(d.importance) * maxBarHeight)
        .attr('height', barWidth)
    }

    const textTokens = textG1.selectAll('text')
      .data(data.tokenScores)
      .join('text')
      .attr('fill', '#fff')
      .text(d => d.token);
    ...
    if (orientation == 'horizontal') {
      textTokens
        .attr('transform', (d, i) => `
          translate(
            ...
            ${rect.height / 2 - d.importance * maxBarHeight - 5 + (d.importance < 0 ? computed.textHeight * 2 + 4 : -computed.textHeight - 2)}
          )
      ...
      textTokens
        ...
        .attr('y', (_, i) => computed.lineStart + i * (barWidth + barMargin) + barMargin / 2)
        .attr('text-anchor', d => d.importance < 0 ? 'start' : 'end')
        .attr('dominant-baseline', 'middle')
    }

    const scoreTokens = textG2.selectAll('text')
      .data(data.tokenScores)
      .join('text')
      .attr('fill', '#fff')
    ...
    if (orientation == 'horizontal') {
      scoreTokens
        .attr('x', (_, i) => computed.lineStart + i * (barWidth + barMargin) + barWidth / 2)
        .attr('y', d => rect.height / 2 - d.importance * maxBarHeight - 5 + (d.importance < 0 ? computed.textHeight + 4 : -2))
        .attr('text-anchor', 'middle')
    } else {
      scoreTokens
        .attr('x', d => rect.width / 2 - d.importance * maxBarHeight + (d.importance < 0 ? 5 : -5))
        .attr('y', (_, i) => computed.lineStart + i * (barWidth + barMargin) + barMargin + barWidth / 2)
        .attr('text-anchor', d => d.importance < 0 ? 'start' : 'end')
        .attr('dominant-baseline', 'middle')
    }
  }, [redraw, data, orientation]);
...
export function LimeSummaryBars(props: LimeSummaryBarsProps) {
  ...
  const svgHeight = barHeight * props.values.length + barMargin * (props.values.length - 1);
  ...
  useEffect(() => {
    ...
    const borderG = svg.append('g')
      .classed('border-group', true);

    svg.append('g')
      .classed('fill-group', true);

    borderG.selectAll('text')
      .data(props.classes)
      .join('text')
      .attr('x', 0)
