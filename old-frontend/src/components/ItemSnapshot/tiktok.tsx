...
function TiktokPreview(props: TiktokPreviewProps) {
  ...
  return (
    <Stack
      ...
      fontFamily: 'TikTokFont, Arial, Tahoma, PingFangSC, sans-serif; !important',
      overflow: 'hidden',
    }}>
      ...
      <Box sx={{ bgcolor: 'white', ...(videoWidth ? { width: videoWidth } : {}) }}>
        <Box sx={{ bgcolor: 'rgba(22, 24, 35, 0.03)', p: '16px' }}>
          <Stack direction="row" spacing="16px">
            ...
            <Stack>
              <Typography sx={{ color: 'black', fontSize: '18px', fontWeight: 'bold' }}>{raw?.authorMeta?.name}</Typography>
              <Typography sx={{ color: 'rgba(22, 24, 35, 0.75)', fontSize: '14px' }}>{raw?.authorMeta?.nickName}</Typography>
            </Stack>
          </Stack>


          <Typography sx={{ color: 'rgb(22, 24, 35)', fontSize: '16px', mt: '16px'}}>
            {raw?.text}
          </Typography>
        </Box>
      </Box>
...
export function TiktokItemSnapshot(props: TiktokItemSnapshotProps) {
  ...
  return (
    <TiktokPreview result={result} noBorder={props.noBorder} />
  );
}
