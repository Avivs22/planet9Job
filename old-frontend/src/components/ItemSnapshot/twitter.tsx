  ...
  return (
    <Box
      sx={{
        ...
        fontFamily: "\"TwitterChirp\", -apple-system, BlinkMacSystemFont, \"Segoe UI\", Roboto, Helvetica, Arial, sans-serif !important",
        p: 2,
        maxWidth: '566px',
      ...
      {tweet?.user && (
        <Stack direction="row" alignItems="center" spacing={2}>
          ...
          <Box>
            ...
            <Typography sx={{ fontSize: '15px', color: 'rgb(113, 118, 123)', lineHeight: '20px' }}>
...
export function TwitterItemSnapshot(props: TwitterItemSnapshotProps) {
  const tweetData = (props.scraper?.result?.raw ?? null) as null | RawTwitterData;

  if (!tweetData)
    return <></>;

  return (
    <TweetBox tweet={tweetData} noBorder={props.noBorder} />
  );
}